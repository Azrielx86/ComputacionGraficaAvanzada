...
vec3 calculateDirectionalLight(Light light, vec3 direction) {
    vec3 colorTexture = texture(texture1, our_uv).rgb;
    vec3 colorTexture2 = texture(texture2, our_uv).rgb;
    vec3 colorFinal = mix(colorTexture, colorTexture2, 0.7f);

    // Ambient
    vec3 ambient = light.ambient * colorFinal;

    // Diffuse 
    vec3 normal = normalize(our_normal);
    vec3 lightDir = normalize(-direction);
    float diff = max(dot(normal, lightDir), 0.0);
    vec3 diffuse = light.diffuse * (diff * colorFinal);
    ...
    vec3 specular = light.specular * (spec * colorFinal);
